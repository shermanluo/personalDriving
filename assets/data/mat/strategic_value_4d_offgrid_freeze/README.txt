...
- Fix: for points that are outside of the grid, evaluate their reward and multiply it by the number of remaining time steps to approximate the strategic value.
...
- For points outside the grid, their reward is multiplied by the number of remaining time steps and is then added to the reward of the point from which the car exited the grid. This causes the robot to wantto be on the edge of the grid corresponding to a large y relative value.